...
const ParallaxScene = () => {
  ...
  const birdSize = isMobile ? 'w-24 h-24' : 'w-32 h-32 md:w-48 md:h-48';
  const textSizeH1 = isMobile ? 'text-3xl' : 'text-4xl md:text-6xl';
  const textSizeP = isMobile ? 'text-base' : 'text-lg md:text-xl';

  // Moon
  const moonY = useTransform(scrollYProgress, [0, 0.5, 1], isMobile ? ['5vh', '15vh', '25vh'] : ['10vh', '20vh', '30vh']);
  const moonX = useTransform(scrollYProgress, [0, 1], isMobile ? ['50vw', '30vw'] : ['60vw', '40vw']);
  ...
  // Mountains
  const mountainsY = useTransform(scrollYProgress, [0, 0.5, 1], isMobile ? ['5%', '20%', '15%'] : ['10%', '30%', '20%']);
  ...
  // Leaves
  const leavesScale = useTransform(scrollYProgress, [0, 0.5, 1], isMobile ? [1, 1.5, 2] : [1, 1.75, 2.5]);
  const leavesY = useTransform(scrollYProgress, [0, 0.5, 1], isMobile ? ['-15%', '-10%', '-5%'] : ['-25%', '-15%', '-5%']);
  const leavesOpacity = useTransform(scrollYProgress, [0, 0.7, 1], [1, 0.9, 0.8]);

  // Bird
  const birdX = useTransform(scrollYProgress, [0, 0.5, 1], isMobile ? ['60vw', '15vw', '-5vw'] : ['80vw', '20vw', '-10vw']);
  const birdY = useTransform(scrollYProgress, [0, 0.5, 1], isMobile ? ['60vh', '15vh', '-5vh'] : ['80vh', '20vh', '-10vh']);
  const birdRotate = useTransform(scrollYProgress, [0, 0.5, 1], [15, 0, -15]);
  const birdScale = useTransform(scrollYProgress, [0, 0.5, 1], isMobile ? [0.7, 1, 1.2] : [0.8, 1.2, 1.5]);
  ...
  return (
    <div ref={ref} className="relative h-[200vh] bg-gradient-to-b from-black via-indigo-900 to-blue-950 overflow-hidden parallax-container">
      ...
      <motion.div
        ...
        animate={{ filter: ['blur(2px)', 'blur(4px)'], transition: { repeat: Infinity, duration: 3, ease: 'easeInOut' } }}
        className={`parallax-element absolute top-0 left-0 transform -translate-x-1/2 z-10 ${moonSize}`}
      >
        <img src={moon} alt="moon" className="w-full h-full drop-shadow-[0_0_15px_rgba(255,255,255,0.5)]" loading="lazy" />
      </motion.div>

      {/* Mountains */}
      <motion.div
        style={{ y: mountainsY, scale: mountainsScale, opacity: mountainsOpacity }}
      ...
      <motion.div
        style={{
          ...
          opacity: useTransform(scrollYProgress, [0, 0.5, 1], [0.3, 0.4, 0.2]),
        }}
      ...
      <motion.div
        style={{
          opacity: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], [0, 0.1, 0.2, 0.05]),
          y: useTransform(scrollYProgress, [0, 1], isMobile ? ['40%', '15%'] : ['50%', '20%']),
        }}
      ...
      >
        <img src={leaves} alt="leaves" className="w-full object-contain" loading="lazy" />
      </motion.div>
      ...
      >
        <img src={bird} alt="bird" className="w-full h-full object-contain" loading="lazy" />
      </motion.div>
        ...
        <div className="content-overlay sticky top-1/3 text-center px-4 sm:px-6">
          <motion.h1
            style={{
              ...
              y: useTransform(scrollYProgress, [0, 0.2, 0.8, 1], isMobile ? ['30px', '0px', '0px', '-30px'] : ['50px', '0px', '0px', '-50px']),
            }}
            className={`parallax-element ${textSizeH1} font-bold mb-4 text-white drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]`}
          >
            After Midnight
          </motion.h1>
          <motion.p
            style={{
              ...
              y: useTransform(scrollYProgress, [0, 0.3, 0.7, 1], isMobile ? ['20px', '0px', '0px', '-20px'] : ['30px', '0px', '0px', '-30px']),
            }}
